-- Corrige l'accès service_role + expose des RPC sécurisées pour l'API Next.js.
...
create or replace function public.employee_account_get(
  p_company_id uuid,
  p_employe_id text
)
...
begin
  ...
  on conflict (company_id, employe_id) do update
  set
...
create or replace function public.employee_account_delete(
  p_company_id uuid,
  p_employe_id text
)
returns void
...
create or replace function public.employee_account_set_active(
  p_company_id uuid,
  p_employe_id text,
  p_active boolean
)
...
as $$
  ...
  set
    employee_account_active = coalesce(p_active, true),
    updated_at = now()
  where company_id = p_company_id
    and employe_id = p_employe_id;
$$;
...
create or replace function public.employee_account_login_taken(
  ...
  p_exclude_employe_id text default null
)
returns boolean
language sql
security definer
...
as $$
  select exists (
    ...
    where lower(trim(employee_login)) = lower(trim(p_login))
      and not (
        p_exclude_company_id is not null
        and p_exclude_employe_id is not null
        and company_id = p_exclude_company_id
        and employe_id = p_exclude_employe_id
      )
  );
$$;
