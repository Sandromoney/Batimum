  ...
  return (
    ...
    >
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto px-3 py-3">
          ...
          >
            <span
              className={cn(
                ...
                message.role === "user" ? "bg-accent/5" : "bg-slate-100",
              )}
            >
              {message.role === "user" ? (
                <User className="h-3.5 w-3.5 text-accent-hover" />
              ) : (
        ...
        {pendingAction ? (
          <div
            ...
            className="rounded-2xl border border-accent bg-accent/5/50 p-3"
          >
        ...
        <div className="flex flex-wrap gap-1.5 border-t border-border/60 px-3 py-2">
          {WELCOME_SUGGESTIONS.map((item) => (
            <button
              key={item}
              type="button"
              className="rounded-full border border-border/70 bg-white px-2.5 py-1 text-[11px] text-slate-600 hover:border-accent/30 hover:text-accent"
      ...
      <footer className="border-t border-border/60 p-3">
        {listening ? (
          <p className="mb-1.5 text-[11px] font-medium text-accent-hover">
            Écoute en cours…
          </p>
        ) : null}
        <div className="flex items-end gap-1.5">
          ...
          <textarea
            ...
            className="min-h-[2.5rem] flex-1 resize-none rounded-xl border border-border/70 bg-white px-3 py-2 text-[13px] outline-none focus:border-accent/30 focus:ring-2 focus:ring-accent disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-muted-foreground"
          />
          <Button
            type="button"
            size="sm"
